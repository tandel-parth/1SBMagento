...
class RocketWeb_GoogleBaseFeedGenerator_Block_Adminhtml_System_Process extends Mage_Adminhtml_Block_Abstract
{
    ...
    protected function runUpdate()
    {
		...
		$feed_data = array();
		if ($this->getIsFeed() && $count_products == 1 && file_exists($Generator->getFeedPath()))
		{
			/* tsv file */
			...
			{
				$feed_data = array_combine($rows[0], $rows[1]);
			}
		}
		$this->setFeedData($feed_data);
		$messages[] = array('msg' => sprintf("The feed was generated.<br />%d products were added %d products were skipped.", $count_products, $count_skipped), 'type' => 'info');
